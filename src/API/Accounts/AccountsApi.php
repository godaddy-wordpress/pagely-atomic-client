...
namespace Pagely\Client\AccountsApi;

use GuzzleHttp\Exception\ClientException;
use Pagely\Client\BaseApiClient;

class AccountsApi extends BaseApiClient
{
    ...
    public function syncBillingCcs($accessToken, $accountId)
    {
        return $this->guzzle($this->getBearerTokenMiddleware($accessToken))->post("accounts/{$accountId}/syncbillingccs");
    }
    ...
    public function transferAccountToCollaborator(
        string $accessToken,
        int $accountId,
        int $collabId,
        string $accountEmail,
    ...
    public function createAccountFromBillingMinimal(
        string $token,
        string $email,
        string $firstName,
        string $lastName,
        ...
        ?string $limits = null,
        ?string $ventureId = null,
        ?string $planId = null,
        bool $addBillingRecords = false,
        bool $isPreProvisioning = false,
        ?string $cellId = null
    ) {
        $body = [
            'email' => $email,
        ...
        foreach (array_filter([
            ...
            'planId' => $planId,
            'limits' => $limits,
            'addBillingRecords' => $addBillingRecords,
        ]) as $key => $value) {
            $body[$key] = $value;
        }

        if ($isPreProvisioning) {
            $body['isPreProvisioning'] = $isPreProvisioning;
        }
        ...
        return $this->guzzle($this->getBearerTokenMiddleware($token))
            ->post('accounts/from-billing-minimal', [
                'json' => $body,
            ]);
    }

    public function createAccountFromBilling(
        string $token,
        string $email,
        string $firstName,
        string $lastName,
        ...
        int $billingTotalAmount = 0,
        string $billingCycle = '',
        string $region = ''
    ) {
        return $this->guzzle($this->getBearerTokenMiddleware($token))
            ->post('accounts/from-billing', [
                'json' => [
                    'email' => $email,
                    'firstName' => $firstName,
                    'lastName' => $lastName,
                    ...
                    'billingTotalAmount' => $billingTotalAmount,
